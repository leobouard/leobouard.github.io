---
title: "LM Hash dans Active Directory"
...
### Conflit avec LMCompatibilityLevel

Si après vos modifications et le redémarrage de vos contrôleurs de domaine, le LM Hash est toujours généré il va falloir regarder du côté du paramètre LMCompatibilityLevel. Si celui-ci est configuré à 0 ou 1, le LM Hash continuera d'être généré (même si la clé de registre `NoLmHash` est active).

- Chemin : *Computer configuration > Policies > Windows Settings > Security Settings > Local Policies > Security Options*
- Paramètre : **Network security: LAN Manager authentication level**

Voici un rappel des valeurs possibles :
...
4	| Send NTLMv2 respons only. Refuse LM
5	| Send NTLMv2 respons only. Refuse LM & NTLM

Ici aussi, la modification prendra effet uniquement après le redémarrage des contrôleurs de domaine.

Plus d'informations sur le sujet : [Des problèmes de client, de service et de programme peuvent se produire si vous modifiez les paramètres de sécurité et les attributions de droits utilisateur - Support Microsoft](https://support.microsoft.com/fr-fr/topic/des-probl%C3%A8mes-de-client-de-service-et-de-programme-peuvent-se-produire-si-vous-modifiez-les-param%C3%A8tres-de-s%C3%A9curit%C3%A9-et-les-attributions-de-droits-utilisateur-0cb6901b-dcbf-d1a9-e9ea-f1b49a56d53a)
